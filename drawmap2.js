...
var paletteScale = d3.scale.linear()
            ...
            .range(["#cf270e","#f03b20","#f57a68","#ffeda0","#66ff66","#33cc33","#009933"]);
			
// loop thorgh all the data to initialize a variable to fit DataMap format.
    for (var i = 0, len = data.length; i < len; i++){
		
		var iso = data[i].codes;
        var value = data[i].polarity;
		var tweets = data[i].tweets
        dataset[iso] = { polarity: Number(data[i].polarity).toFixed(2), fillColor: paletteScale(value), numberOfTweets: tweets};
    };
// Create a datamap Object
	 var datamap = new Datamap({
		...
		// event handler wating for a click on a country to be fired
		done: function(datamap) {
            datamap.svg.selectAll('.datamaps-subunit').on('click', function(data) {
				// popup
				swal({
				title: " Selected: " + data.properties.name + "",
				text: "Loading, Please wait.",
				timer: 2000,
				showConfirmButton: false
				});
				//read data to populate clouds and summary stats based on selection
				d3.csv("perfectlocations.csv", function(ndata){
				...
				{
				if(Number(ndata[i].polarity) < 0 && data.id == ndata[i].codes){
					neg ++;
				} else if (Number(ndata[i].polarity) > 0 && data.id == ndata[i].codes){
					pos ++;
				} else if (Number(ndata[i].polarity) == 0 && data.id == ndata[i].codes){
					tneu ++;
				}
				...
				document.getElementById("selection").innerHTML= data.properties.name;
				
d3.csv("perfectlocations.csv", function(worddata){
// get only the selected country
worddata = worddata.filter(function(d) {return d.codes == data.id;})
// clear the word clouds drawing areas
document.getElementById("WordCloud").innerHTML = "";
document.getElementById("WordCloud-pos").innerHTML = "";
document.getElementById("WordCloud-neg").innerHTML = "";
var mywords = "";

for (var i = 0, len = worddata.length; i < len; i++) 
{
	mywords += JSON.stringify(worddata[i].text);
}
// convert the text into words and remove stop words
mywords = text2wordmap(mywords);
// only more than 50 frequency
mywords.filter(function(d) {return d.value > 50;})

var fill = d3.scale.category20();
var cloud = d3.layout.cloud;
// initialize the full cloud
var layout = cloud()
    .size([800, 500])
...
for (var i = 0, len = worddata.length; i < len; i++) 
{
	if(Number(worddata[i].polarity) < 0){
		negwords += JSON.stringify(worddata[i].text);;
	} if (Number(worddata[i].polarity) > 0){
		poswords += JSON.stringify(worddata[i].text);;
	}
}
// convert the text into words and remove stop words
...
//initialize pos cloud
var fill = d3.scale.linear()
    .domain([0,10,20,30,50])
    .range(['#00441b','#006d2c','#238b45','#41ab5d','#41ab5d']);
var cloud = d3.layout.cloud;
...
//initialize neg cloud
var filln = d3.scale.linear()
    .domain([10,20,30,30,50])
    .range(['#bd0026','#e31a1c','#fc4e2a','#fd8d3c','#fd8d3c']);
var cloud = d3.layout.cloud;
...
//Detailed clouds end.
	...
	datamap.legend({
		...
		//draw bubbles for number of tweets
		var bubble = [];
